...
<script type="text/javascript">
	function searchUser(){
		window.location.href="${pageContext.request.contextPath}/manage/list?pageCurrent="+${pageBean.pageCurrent}+"&queryUserName="+$("#inputUser").val();
	}
...
<script type="text/javascript">
	function datadel(){
		...
		$("#JvForm").attr("action","${pageContext.request.contextPath}/manage/deleteByIds");
...
<div class="page-container">
	<div class="text-c"> 
		<input type="text" name="" value="${queryUserName}" id="inputUser" placeholder="管理员用户名" style="width:250px" class="input-text">
		<button name="" id="searchUser" onclick="searchUser();" class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i> 搜索管理员</button>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l">
		<a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 状态修改</a> 
		
		<a class="btn btn-primary radius" data-title="添加管理员" _href="<%=path%>/manage/add" onclick="Hui_admin_tab(this)" href="javascript:;">
	...
	<form id="JvForm">
			...
			<c:forEach items="${pageBean.datas}" var="user">
				<tr class="text-c">
					...
					<c:if test="${user.countState == 2 }">
						<td class="td-status"><span class="label label-danger radius">不可用</span></td>
					</c:if>		
					
					<td class="f-14 td-manage">					
					 
						<a style="text-decoration:none" class="ml-5" data-title="修改" _href="<%=path %>/manage/toEdit?id=${user.id}" onclick="Hui_admin_tab(this)" href="javascript:;"  title="修改权限">
		...
		<div class="text-c" id="page1">
			<div id="laypage_0" class="laypage_main laypageskin_default" name="laypage1.2">
				...
				<c:if test="${pageBean.pageCurrent != 1}">
					<a data-page="27" class="laypage_prev" href="javascript:void(0)" onclick="window.location.href='<%=path %>/manage/list?pageCurrent=1&queryUserName=${queryUserName}'">首页</a>
				</c:if>
				
				<c:if test="${pageBean.pageCurrent > 1}">
					<a data-page="27" class="laypage_prev" href="javascript:void(0)" onclick="window.location.href='<%=path %>/manage/list?pageCurrent=${pageBean.pageCurrent-1}&queryUserName=${queryUserName}'">上一页</a>
				</c:if>
		...
		<c:forEach var="i" begin="${begin}" end="${end}">
			<c:choose>
				...
				<c:otherwise>
					<a data-page="${i}" class="laypage_last" href="javascript:void(0)" onclick="window.location.href='<%=path %>/manage/list?pageCurrent=${i}&queryUserName=${queryUserName}'">${i}</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
				
				<c:if test="${pageBean.pageCurrent < pageBean.totalPage}">
					<a data-page="29" class="laypage_next" href="javascript:void(0)" onclick="window.location.href='<%=path %>/manage/list?pageCurrent=${pageBean.pageCurrent+1}&queryUserName=${queryUserName}'">下一页</a>
				</c:if>
				
				<c:if test="${pageBean.pageCurrent != pageBean.totalPage}">
					<a data-page="29" class="laypage_next" href="javascript:void(0)" onclick="window.location.href='<%=path %>/manage/list?pageCurrent=${pageBean.totalPage}&queryUserName=${queryUserName}'">尾页</a>
				</c:if>
